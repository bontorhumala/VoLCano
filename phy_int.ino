...
#ifdef DEBUG_TX
    Serial.print(F("eb0: ")); Serial.print(encode_buffer[0]);Serial.print(F(", eb1: ")); Serial.print(encode_buffer[1]);Serial.print(F(", ei: "));Serial.print(encode_iter & 0x01);Serial.print(F(", tx: ")); Serial.println(encode_buffer[encode_iter & 0x01]);
#endif
...
#ifdef DEBUG
    Serial.println(F("corrupt, PHY_IDLE"));
#endif
    _reset_rx_to_idle();
    phy_state = PHY_IDLE;
    return;
...
void _phy_rx() {
  ...
  if ((phy_state == PHY_RX) || (phy_state == PHY_PREAMBLE_RX)) {
    Serial.print(micros()-time_rx);Serial.print(F(", "));
    time_rx = micros();
  }
#endif
  if (in_bit > -1) { // check incoming bit
...
#ifdef DEBUG_RX
      Serial.print(F("OK, 1 byte: "));Serial.println(rx_buffer[rx_iter]);
#endif
...
void _phy_preamble_tx() {
#ifdef DEBUG_TX
  Serial.print(F("PRETX: "));Serial.print(encode_iter & 0x01);Serial.print(F(", prei: "));Serial.print(preamble_iter);Serial.print(F(", puli: "));Serial.println(pulse_iter);
#endif
...
void _phy_tx_rx() {
#ifdef DEBUG_TX
  Serial.print(F("TXRX: "));Serial.print(encode_iter & 0x01);Serial.print(F(", tx_bits_iter: "));Serial.print(tx_bits_iter);Serial.print(F(", tx_iter: "));Serial.print(tx_iter);Serial.print(F(", puli: "));Serial.println(pulse_iter);
#endif
  if (((encode_iter & 0x01) == 0) && (pulse_iter == PULSE_LEN)) { // need to update encode_buffer at the start of a bit (end of encode_buffer)
    if (tx_bits_iter == 0) { // start of a byte, convert byte to bits
      ...
      if (tx_iter == tx_len) { // finished transmission
        ...
        phy_state = PHY_IDLE;
#ifdef DEBUG
        Serial.println(F("Finish transmission, go to IDLE"));
#endif
        return;
      }
      tx_iter++;
...
int8_t _detect_edge() {
...
#ifdef DE_PROFILING
  if ((phy_state == PHY_RX) || (phy_state == PHY_PREAMBLE_RX)) {
    Serial.print(micros()-time_de);Serial.print(F(", "));
    time_de = micros();
  }
#endif
  ...
  if ((phy_state == PHY_RX) || (phy_state == PHY_PREAMBLE_RX)) {
    ...
    for (uint8_t j = 0; j < PULSE_LEN; j++) {
      Serial.print(sampling_buffer[j]); Serial.print(F(", "));
    }
