...
const String stickers[] = {"BQADAgADvwADlIB3BSO3_Hx3PVABAg",   // Meow Nya Sticker - 0
  ...
  "BQADAgAEBQACYyviCQXAY0t2nQWlAg", // Bye obama - 6
  ...
  if(buf_in_len > 0)
  {
    ...
    switch (cmd_in) {
      ...
      case CMD_STATUS_REPLY:
        ...
        switch(buf_in[0]) {
          case NODE_1_ID:
            bot.sendMessage(pending_chat_id, "Node " + String(NODE_1_ID) + " -- OK\n", "");
            break;
          case NODE_2_ID:
            bot.sendMessage(pending_chat_id, "Node " + String(NODE_2_ID) + " -- OK\n", "");
            break;
          default:
            bot.sendMessage(pending_chat_id, "Receiving from invalid node ID\n", "");
            break;
        }
        break;
      case CMD_SPEEDTEST_REPLY:
        String msg;
        int16_t speed_in = (buf_in[2] << 8) | buf_in[3];
        msg = "Node-" + String(buf_in[0], DEC) + " to Node-" + String(buf_in[1], DEC) + " (avg): " + String(speed_in, DEC) + " bps\n";
        bot.sendMessage(pending_chat_id, msg, "");
        break;
      // default:
      //   break;
    }
  ...
  for(int i=0; i<numNewMessages; i++) {
    ...
    switch(telegram_state)
    {
      ...
      case WAIT_TELE_ARG_1:
        ...
        {
          if(text == "0") {
            bot.sendMessage(chat_id, "Node " + String(NODE_GATEWAY_ID) + " (GATEWAY) -- OK \n", "");
            sendDefaultMessage(chat_id);
          } else {
            char buff[] = {NODE_GATEWAY_ID, text.toInt()};
