/*
  VolcanoMAC.cpp - Library for for MAC in VoLCano: visible light to light communication.
  Created by Bontor Humala, October 28, 2016.
  
*/

#include "Arduino.h"
#include "VolcanoMAC.h"
...
void VolcanoMAC::_mac_fsm_control() {
  // read phy
  _mac_read_packet();
  switch(mac_state){
    case MAC_IDLE:
...
void VolcanoMAC::_mac_read_packet() {
...
void VolcanoMAC::_wait_ack_slot() {
...
void VolcanoMAC::_wait_cw_slot(uint8_t num_cw) {
...
void VolcanoMAC::_mac_create_ack(uint8_t mac_ack_pdu[], uint8_t ack_dest_addr, uint8_t first_byte) {
  uint16_t fcs;
...
uint16_t VolcanoMAC::_mac_create_pdu(uint8_t mac_pdu[], uint8_t tx_type) {
  uint16_t fcs;
  uint8_t i;
  uint16_t data_len;
  uint8_t tx_addr;
...
// Calculate CRC
uint16_t VolcanoMAC::_calculate_fcs(uint8_t* data, uint8_t count)
{
  uint16_t result;
  result = 0;
