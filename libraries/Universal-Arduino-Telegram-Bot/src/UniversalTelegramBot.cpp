...
bool UniversalTelegramBot::sendSticker(String chat_id, String sticker)  {
  ...
  return sendPostSticker(payload);
}

bool UniversalTelegramBot::sendMessageWithReplyKeyboard(String chat_id, String text, String parse_mode, String keyboard, bool resize, bool oneTime, bool selective, bool force_reply)  {
...
/***********************************************************************
* SendPostSticker - function to send sticker to telegram                  *
* (Arguments to pass: chat_id, sticker to transmit and markup(optional)) *
***********************************************************************/
bool UniversalTelegramBot::sendPostSticker(JsonObject& payload)  {

  bool sent=false;
  Serial.println("SEND Post Sticker ");
  long sttime=millis();
  if (payload.containsKey("sticker")) {
    while (millis()<sttime+8000) {    // loop for a while to send the message
      String command = "bot"+_token+"/sendSticker";
      String response = sendPostToTelegram(command, payload);
      Serial.println(response);
      sent = checkForOkResponse(response);
      if(sent){
